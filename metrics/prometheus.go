...
func (p *PrometheusConfig) ingest(metric interface{}){
	offsetMetric := metric.(PrometheusMetric)
	switch offsetMetric.MetricType {
	case prometheus.GaugeValue:
		p.offsetMetrics.WithLabelValues(offsetMetric.MetricName).Set(float64(offsetMetric.MetricValue))
	}
}
